...
var _spr, _wid, _autoalpha, _color, _xt, _xt_i, _a, _px, _py, _nx, _ny, i, _size, _dir, _dx, _dy, _trailAlpha, _finalAlpha, _idx;
_spr = argument0;
_wid = argument1 * 0.5;
...
_size = ds_list_size(trailList) div 3;
if (_size <= 0) exit;

_px = ds_list_find_value(trailList, 0);
_py = ds_list_find_value(trailList, 1);
draw_primitive_begin_texture(pr_trianglestrip, sprite_get_texture(_spr, 0));

for (i = 0; i < _size; i += 1) {
    _idx = i * 3;
    _nx = ds_list_find_value(trailList, _idx);
    _ny = ds_list_find_value(trailList, _idx + 1);
    _trailAlpha = ds_list_find_value(trailList, _idx + 2);
