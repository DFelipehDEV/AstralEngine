...
if (distance_to_object(objRing) > 90 || !instance_exists(objRing) || ground) {
    ...
    if (instance_exists(_nearRing)) {
        ...
        xSpeed = lerp(xSpeed, lengthdir_x(lightspeedSpeed, _ringDir), 0.4 * global.timeScale);
        ySpeed = lerp(ySpeed, lengthdir_y(lightspeedSpeed, _ringDir), 0.4 * global.timeScale);
