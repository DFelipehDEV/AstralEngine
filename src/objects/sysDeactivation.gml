#define Create_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
sysDeactivation.deactivateExceptions = dss_list_create();
#define Step_1
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if (GameStateGet() == GameStatePaused) exit;
        instance_deactivate_all(true);
        instance_activate_object(gm82core_object);
        instance_activate_object(objAlwaysActive);

        // Activate specific instances
        var i, _instance;
        for (i = 0; i < ds_list_size(sysDeactivation.deactivateExceptions); i += 1) {
            _instance = ds_list_find_value(sysDeactivation.deactivateExceptions, i);
            instance_activate_object(_instance);

            if (!instance_exists(_instance)) DeactivateExceptionsRemove(_instance);
        }

        instance_activate_region(view_xview[0] - 64, view_yview[0] - 32, view_wview[0] + 96, view_hview[0] + 96, 1);
#define Other_5
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
ds_list_clear(sysDeactivation.deactivateExceptions);
#define Draw_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
BeginUI();
        if (debug_mode) draw_text(32, 32, string(ds_list_size(sysDeactivation.deactivateExceptions)));
        EndUI();
